...
func (c *Controller) syncSensuHandler(handler *api.SensuHandler) {
	c.logger.Warnf("in syncSensuHandler, about to update checkconfig within sensu cluster")
	// TODO need cluster namespace in handler CRD
	sensuClient := sensu_client.New(handler.ClusterName, handler.GetNamespace(), "default")
	err := sensuClient.UpdateHandler(handler)
	c.logger.Warnf("in syncSensuHandler, after update handler in sensu cluster")
	if err != nil {
		c.logger.Warningf("failed to handle handler update event: %v", err)
	}
	copy := handler.DeepCopy()
	copy.Status.Accepted = true
	c.logger.Warnf("in syncSensuHandler, about to update handler status within k8s")
	...
	c.logger.Warnf("in syncSensuHandler, done updating handler status within k8s")
}
